...
org_rank_furbo = []
spons_rank_furbo = []
search_volume_furbo_free = []

for i in range(0, len(keywordsf)):
    if search_volume_furbo[i] > 300:
        if organic_rank_furbo[i] < 15 and organic_rank_furbo[i] > 0:
            if sponsored_rank_furbo[i] > 5 or sponsored_rank_furbo[i] == 0:
                keywords_furbo.append(keywordsf[i])
                org_rank_furbo.append(organic_rank_furbo[i])
                spons_rank_furbo.append(sponsored_rank_furbo[i])
                search_volume_furbo_free.append(search_volume_furbo[i])

for volume in search_volume_furbo_free:
    print(volume)
print(len(search_volume_furbo_free))